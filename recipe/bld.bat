...
if "%PKG_NAME%" == "pytorch" (
  ...
  sed "s/\([Pp]ython\)312/\1%CONDA_PY%/g" original.txt > output.txt

  :: Replace version string v3.12.8() with ie v3.11.11()
  sed -i.bak -E "s/v3\.12\.[0-9]+/v%PY_VERSION_FULL%/g" output.txt

  :: Replace interpreter properties Python;3;12;8;64 with ie Python;3;11;11;64
  :: sed -i.bak -E "s/Python;3;12;[0-9]+;64/Python;%PY_VERSION_FULL%//./;};64/g" output.txt
  sed -i.bak -E "s/Python;3;12;[0-9]+;64/Python;%PY_VERSION_FULL:.=;%;64/g" output.txt

  :: Replace cp312-win_amd64 with ie cp311-win_amd64
  sed -i.bak "s/cp312/cp311/g" output.txt
